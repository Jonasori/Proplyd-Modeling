...
def plot_gridSearch_log(fname, show=False):
    ...
    axarr[(0, 0)].axis('off')
    axarr[(0, 1)].axis('off')
    axarr[(0, 0)].text(0.2, -0.2, 'Summary of\n' + run_date + ' Run', fontsize=16, fontweight='bold')
    ...
    for d in [0, 1]:
        ...
        for i, p in enumerate(params, 1):
            ...
            for bf in p['best_fits']:
                ...
                axarr[(i, d)].plot(bf, 0, marker='o', markersize=10, color='black', alpha=a)
                axarr[(i, d)].plot(bf, 0, marker='o', markersize=9, color=colors[d], markerfacecolor='none', markeredgewidth=3)
...
def plot_step_duration(dataPath, ns=[10, 20, 50], show=False):
    ...
    for i in range(len(ns)):
        ...
        plt.plot(xs[n / 2:-n / 2], avg_ys, linestyle='-', color=colors[i], linewidth=0.1 * n, label=str(n) + '-step smoothing')

    run_date = dataPath.split('/')[-1]
    plt.legend()
    plt.xlabel('Step', fontweight='bold')
    plt.ylabel('Time (minutes)', fontweight='bold')
    plt.title('Time per Step for Grid Search Run on ' + run_date, fontweight='bold', fontsize=14)
...
def full_analysis_plot(pickleLog, timeLog):
    ...
    inner = gridspec.GridSpecFromSubplotSpec(len(both_disks[0]), 2, subplot_spec=outer[1], wspace=0.1, hspace=1)
    for d in range(2):
        params = both_disks[d]
        for i, p in enumerate(params):
            ...
            for bf in p['best_fits']:
                ax.plot(bf, 0, marker='o', markersize=9, color=colors[d], markerfacecolor='none', markeredgewidth=2)
    ...
    for i in range(len(ns)):
        ...
        ax_bottom.plot(steps[n / 2:-n / 2], avg_ys, linestyle='-', color=colors[i], linewidth=0.1 * n, label=str(n) + '-step smoothing')

    ax_bottom.legend()
    ax_bottom.set_xlabel('Step', fontweight='bold')
    ax_bottom.set_ylabel('Time (minutes)', fontweight='bold')
    ax_bottom.set_title('Time per Step for Grid Search Run on ' + run_date, fontweight='bold', fontsize=14)
    fig.add_subplot(ax_bottom)
    fig.show()


def plot_fits(image_path, mol=mol, scale_cbar_to_mol=False, crop_arcsec=2, cmap='magma', save=True, show=True, use_cut_baselines=True, best_fit=False):
    """
...
def plot_model_and_data(image_path, mol=mol, scale_cbar_to_mol=False,
    ...
    for i in range(nchans):
        ...
        ax.set_xticklabels([])
        ax.set_yticklabels([])
        ax.text(0, -0.8 * crop_arcsec, velocity + ' km/s', fontsize=6, color='w', horizontalalignment='center', verticalalignment='center')
        if i == n_rows * (n_cols - 1) and add_beam is True:
            el = ellipse(xy=[0.8 * crop_arcsec, 0.8 * crop_arcsec], width=bmin, height=bmaj, angle=-bpa, fc='k', ec='w', fill=False, hatch='////////')
            ax.add_artist(el)
        cmaps = imshow(real_data[i + chan_offset][xmin:xmax, xmin:xmax], cmap=cmap, vmin=vmin, vmax=vmax, extent=(
         crop_arcsec, -crop_arcsec,
         crop_arcsec, -crop_arcsec))
        ax.plot(offsets_dA[0], offsets_dA[1], '+g')
        ax.plot(offsets_dB[0], offsets_dB[1], '+g')
...
def plot_model_and_data_gridspec():
    """Should add residuals as well while I'm at it."""
    plt.close()
